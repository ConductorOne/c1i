...
| 2 | usage error (bad flags/args, an empty id, or any API `4xx` other than `401`/`403`/`404`/`429`) | fix the invocation, don't retry as-is — but read the message first: an API `4xx` can also be a state rejection (e.g. `task is closed`) rather than a bad argument |
